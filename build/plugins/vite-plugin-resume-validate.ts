  ...
  return {
    ...
    async buildStart() {
      ...
      try {
        ...
        console.warn('\n  [resume-validate] Impossible de charger resume-config.ts — vérification ignorée.\n')
        return
      }
      ...
      if (
        ...
        warnings.push(
          '  Vous utilisez encore les données d\'exemple (Jane Doe).\n' +
      ...
      if (config.personal.photo) {
        ...
        if (!fs.existsSync(photoPath)) {
          warnings.push(
            `  L'image "${config.personal.photo}" n'existe pas dans public/.\n` +
        ...
        if (!hasImage) {
          warnings.push(
            '  Aucune image trouvée dans public/images/.\n' +
